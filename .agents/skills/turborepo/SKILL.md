...
In Turborepo `2.0+`, strict mode is the default. In `1.10+`, the same filtering applies when you run with `--env-mode=strict`. CI variables may be missing:
...
Or use `--env-mode=loose` on Turborepo `1.10+` (not recommended for production).
