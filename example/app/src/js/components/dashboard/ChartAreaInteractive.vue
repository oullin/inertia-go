...
import { ToggleGroup, ToggleGroupItem } from "@/js/components/ui/toggle-group";
    ...
    <CardContent class="px-2 pt-4 sm:px-6 sm:pt-6">
      <ChartContainer :config="chartConfig" class="aspect-auto h-[250px] w-full" :cursor="false">
        <VisXYContainer
          ...
          :svg-defs="svgDefs"
          :margin="{ left: -40 }"
          :y-domain="[0, Math.max(...filteredData.map((d) => d.mobile + d.desktop)) * 1.1]"
        >
          <VisArea
            :x="(d) => d.date"
            :y="[(d) => d.mobile, (d) => d.desktop]"
            :color="(_d, i) => ['url(#fillMobile)', 'url(#fillDesktop)'][i]"
            :opacity="0.6"
          />
          <VisLine
            :x="(d) => d.date"
            :y="[(d) => d.mobile, (d) => d.mobile + d.desktop]"
            :color="(_d, i) => [chartConfig.mobile.color, chartConfig.desktop.color][i]"
            :line-width="1"
          />
          <VisAxis
            type="x"
          ...
          <ChartCrosshair
            ...
            "
            :color="(_d, i) => [chartConfig.mobile.color, chartConfig.desktop.color][i % 2]"
          />
        </VisXYContainer>
      </ChartContainer>
    </CardContent>
